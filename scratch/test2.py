...
IP = "192.168.1.51"  # Camera IP address
PORT = 2000  # Port
USER = "admin"  # Username
PASS = "admin"  # Password
...
def do_move(ptz, request):
    ...
    position = ptz.GetStatus({'ProfileToken': request.ProfileToken}).Position
    print('move status:', position)
...
def readin():
    ...
    if lov:
        ...
        if lov[0].lower() in ["u", "up"]:
            ...
            print(
                "What are you asking?\tI only know, 'up','down','left','right', 'ul' (up left), \n\t\t\t'ur' (up right), 'dl' (down left), 'dr' (down right) and 'stop'")
